...
class grid_map(object):
	...
	def __init__(self):
		...
		self.pose = [0,0,0]
		self.robot_size = 0.4		# square with size robot_size		
	...
	def update(self, msg):
		...
		print(now)
		start = time.time()
		self.listener.waitForTransform('/map', '/base_laser_front_link', now, rospy.Duration(10.0))
		end = time.time()
		print("The time ellapsed was: " + str(end-start))
		print("Laser stamp: " + str(msg.header.stamp.secs))
		'''
		...
		base_link_pose = [trans[0], trans[1]]
		(roll, pitch, yaw) = euler_from_quaternion(rot)
		base_link_pose.append(yaw)
		self.free_cells(base_link_pose)
	...
	def free_cells(self, pose):
		""" Set the robot cells to free """
		cell = [0,0]
		cell[0] = int( (pose[0] - (self.width * self.res / 2)) // self.res )
		cell[1] = int( ((self.height * self.res / 2) - pose[1]) // self.res )

		# still have to check if it's inside the map REMEMBER
		num_cells = int( self.robot_size // self.res )
		print(num_cells)
		print(cell)
		self.grid[cell[0] - num_cells : cell[0] + num_cells, cell[1] - num_cells : cell[1] + num_cells] += 10 * self.l_free
