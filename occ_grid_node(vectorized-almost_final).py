...
class grid_map(object):
	...
	def __init__(self):
		...
		self.grid_position_m = np.array(
			[np.tile(np.arange(-self.width*self.res/2.0 + self.res/2.0, self.width*self.res/2.0, self.res), (self.height, 1)),
			 np.tile(np.arange(self.height*self.res/2.0 - self.res/2.0, -self.height*self.res/2.0, -self.res)[:, None], (1, self.width))])
	...
	def update(self, msg):
		...
		try:
			# listen to transform
			self.listener.waitForTransform('/map', '/base_laser_front_link', msg.header.stamp, rospy.Duration(1.0))
			(trans,rot) = self.listener.lookupTransform('/map', '/base_laser_front_link', msg.header.stamp)
		...
		d_m = self.grid_position_m.copy()  # A tensor of coordinates of all cells
		d_m[0, :, :] -= pose[0]  # A matrix of all the x coordinates of the cell
		d_m[1, :, :] -= pose[1]  # A matrix of all the y coordinates of the cell
		theta_to_grid = np.arctan2(d_m[1, :, :], d_m[0, :, :]) - pose[2]  # matrix of all bearings from robot to cell

		dist_to_grid = np.linalg.norm(d_m, axis=0)  # matrix of L2 distance to all cells from robot

		ang = np.arange(msg.angle_min, msg.angle_max + msg.angle_increment, msg.angle_increment)
		
		#free robot cells
		try:
			# listen to transform from base_link to map
			self.listener.waitForTransform('/map', '/base_link', msg.header.stamp, rospy.Duration(1.0))
			(trans,rot) = self.listener.lookupTransform('/map', '/base_link', msg.header.stamp)
		except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
			print("Error looking up transform!")
			return
		d_base_link = self.grid_position_m.copy()  # A tensor of coordinates of all cells
		d_base_link[0, :, :] -= trans[0]  # A matrix of all the x coordinates of the cell
		d_base_link[1, :, :] -= trans[1]  # A matrix of all the y coordinates of the cell
		base_link_dist = np.linalg.norm(d_base_link, axis=0)  # matrix of L2 distance to all cells from robot
		free = base_link_dist < self.size
		self.grid[free] += 10.0*self.l_free
		...
		for k in range(len(msg.ranges)):
			...
			free_mask = (np.abs(ang_dif) <= self.beta / 2.0) & (dist_to_grid < (z - self.alpha / 2.0))
			occ_mask = (np.abs(ang_dif) <= self.beta / 2.0) & (np.abs(dist_to_grid - z) <= self.alpha / 2.0)

			# Adjust the cells appropriately
			self.grid[occ_mask] += self.l_occ
			self.grid[free_mask] += self.l_free
	...
	def publish_map(self):
		...
		for i in range(self.width):
			for j in range(self.height):
				map_msg.data.append( self.norm_map[j,i] )
